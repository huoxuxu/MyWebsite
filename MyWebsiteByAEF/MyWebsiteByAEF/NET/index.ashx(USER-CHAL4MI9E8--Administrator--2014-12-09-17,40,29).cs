...
namespace Website.NET
{
    ...
    public class index : IHttpHandler
    {
        ...
        string successMsg = "{\"success\":true,\"info\":\"\"}";
        ...
        {
            string keyword = hc.Request["key"].Trim().ToLower();
            ...
            try
            {
                 ...
                 */
                QuestionAndAnswerTable qaat = new QuestionAndAnswerTable();
                IEnumerable<questionandanswer> qList = new List<questionandanswer>();
                //IQueryable<questionandanswer> qList1 = null;
                foreach (var item in key)
                {
                    var qu = from a in qaat.SelectManyWithQueryable()
                            where a.Question.ToLower().Contains(item)
                                   || a.Answer.ToLower().Contains(item)
                                   || a.Keyword.ToLower().Contains(item)
                            select a;
                    qList=qList.Concat(qu.ToList());
                }
                qList = qList.Distinct().ToList();
                hc.Response.Write("{\"success\":true,\"info\":\"\",\"total\":" + qaat.SelectManyWithQueryable().Count() + ",\"rows\":" + JsonNetHelper.SerializeObject(qList) + "}");
            }
            catch
            {
                hc.Response.Write(errorMsg);
        ...
        {
            string title = hc.Request.Form["title"].Trim();
            string content = hc.Request.Form["editorValue"].Trim();
            string keyword = hc.Request.Form["keyword"].Trim();
            if (string.IsNullOrWhiteSpace(title) || string.IsNullOrWhiteSpace(content) || string.IsNullOrWhiteSpace(keyword)) { return; }
            try
            {
                QuestionAndAnswerTable idm = new QuestionAndAnswerTable();
                ...
                idm.Add(qaa);
                hc.Response.Write(successMsg);
            }
            catch { hc.Response.Write(errorMsg); }
